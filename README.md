...
📦 **Lumen** (my flagship project) → live at [buildwithkulshresth.com/app](https://buildwithkulshresth.com/app)
A multi-tenant marketing OS: 4-stage async pipeline (RabbitMQ), Redis time-wheel scheduling 1M+ concurrent tasks, provider-agnostic LLM layer, PostgreSQL with row-level security — running in production on Kubernetes.
...
>
> - **4-stage async pipeline** via RabbitMQ (content generation → review → scheduling → delivery)
...
>     - - **Provider-agnostic LLM client** — orgs bring their own OpenAI/Anthropic/Gemini key, encrypted at rest
...
>           > - - **Event-driven by default** for async systems (decoupled, independently scalable)
>           >   - - **Intentional tradeoffs**: at-least-once + idempotent consumers over exactly-once complexity; row-level security over schema-per-tenant; bring-your-own-AI over vendor lock-in
>           >    
>           >     - ---
>           >
>           > ## Stack
...
>           >
>           > 📧 jangidkulshresth@gmail.com · [LinkedIn](https://linkedin.com/in/kulshresth-jangid) · [buildwithkulshresth.com](https://buildwithkulshresth.com)
>           > 
